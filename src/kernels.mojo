        ...
        if used < k:
            ...
            while child > 0:
                ...
                if not _worse(
                    ...
                    break
                _swap_ranked(dst_docs, dst_scores, child, parent)
                child = parent
        elif score > dst_scores[0] or (
            score == dst_scores[0] and doc < dst_docs[0]
        ):
            dst_docs[0] = doc
            dst_scores[0] = score
            _sift_down(dst_docs, dst_scores, 0, used)
    var size = used
    while size > 1:
